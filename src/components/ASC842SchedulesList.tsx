...
export function ASC842SchedulesList() {
  ...
  const handleExport = async (leaseId: number) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/schedules/asc842/${leaseId}?format=excel`,
        {
          headers: await getHeaders(),
          method: 'GET',
        }
      ...
      link.href = url;
      link.setAttribute('download', `ASC842_Schedule_${leaseId}.xlsx`);
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>ASC 842 Compliance Schedules</CardTitle>
          <CardDescription>
            View and manage ASC 842 lease schedules across all contracts
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {paginatedLeases && paginatedLeases.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={4} className="py-8 text-center text-muted-foreground">
                      No leases found. Create a lease to generate ASC 842 schedules.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
